...
import {
  ...
  clearFactionShipCargo,
  depositFactionResourcesToSystem,
  extractFactionResourceNodeYield,
...
import {
  ...
  getSpaceCellKeysWithinDistance,
...
type PrimeWorldResourceRequest = {
  planetId: string;
  systemId: string;
  templateId: FactionResourceType;
  amount: number;
...
type SpaceFactionShip = {
  ...
  miningState: FactionForceMinerState | null;
  targetResourceNodeId: string | null;
  cargoCarrierShipId: string | null;
  miningProgress: number;
  miningBeam: Phaser.GameObjects.Line | null;
};
...
const WORLD_SYNC_INTERVAL_MS = 90;
const WORLD_SYNC_HYPERDRIVE_INTERVAL_MS = 32;
const RADAR_REFRESH_INTERVAL_MS = 110;
const COLLISION_UPDATE_INTERVAL_MS = 34;
...
const ACTIVE_FIELD_STREAM_DISTANCE = 2200;
const ACTIVE_SHIP_STREAM_DISTANCE = 2600;
const ACTIVE_CELESTIAL_STREAM_DISTANCE = 3600;
...
const MINER_SLOT_PADDING = 12;
const MINER_SLOT_PADDING_LARGE = 20;
const MINER_SLOT_RING_STEP = 18;
...
const SPACE_COMBAT_DETAIL_RADIUS = 2200;
const SPACE_COLLISION_DETAIL_RADIUS = 1560;
const SPACE_AVOIDANCE_DETAIL_RADIUS = 1480;
const FLEET_AVOIDANCE_RADIUS = 196;
const ASTEROID_AVOIDANCE_RADIUS = 220;
const FORMATION_SLOT_LOCK_DISTANCE = 64;
...
function getActiveSpatialBucketKey(x: number, y: number): string {
  return `${Math.floor(x / ACTIVE_SPATIAL_BUCKET_SIZE)},${Math.floor(y / ACTIVE_SPATIAL_BUCKET_SIZE)}`;
}

function getActiveSpatialBucketKeysAround(x: number, y: number, radius: number): string[] {
  const minX = Math.floor((x - radius) / ACTIVE_SPATIAL_BUCKET_SIZE);
  const maxX = Math.floor((x + radius) / ACTIVE_SPATIAL_BUCKET_SIZE);
  const minY = Math.floor((y - radius) / ACTIVE_SPATIAL_BUCKET_SIZE);
  const maxY = Math.floor((y + radius) / ACTIVE_SPATIAL_BUCKET_SIZE);
  const keys: string[] = [];
  for (let bucketY = minY; bucketY <= maxY; bucketY += 1) {
    for (let bucketX = minX; bucketX <= maxX; bucketX += 1) {
      keys.push(`${bucketX},${bucketY}`);
    }
  }
  return keys;
}
...
function createRockPoints(radius: number, seedKey: string): number[] {
  ...
  return points.flatMap((point) => [point.x - centroidX, point.y - centroidY]);
}
...
export class SpaceScene extends Phaser.Scene {
  ...
  private galaxyZonesById = new Map<string, GalaxyZoneRecord>();
  ...
  private resourceNodesByFieldId = new Map<string, FactionResourceNodeRecord>();
  ...
  private activeFactionShipsById = new Map<string, SpaceFactionShip>();
  private minerAssignedShipIdsByNodeId = new Map<string, string[]>();
  ...
  private worldSyncTimerMs = 0;
  private lastWorldSyncCellKey = "";
  private radarRefreshTimerMs = 0;
  private collisionUpdateTimerMs = 0;
  private cachedRadarSources: SpaceRadarContactSource[] = [];
  ...
  private primeWorldResourceRequest: PrimeWorldResourceRequest | null = null;
  ...
  create(): void {
    ...
    this.forceState.resourceNodes.forEach((node) => {
      this.resourceNodesByFieldId.set(node.fieldId, node);
    });
    ...
    this.primeWorldResourceRequest = null;
    this.liveMissionGrantTimerMs = LIVE_MISSION_GRANT_DELAY_MS;
    this.burstParticles = [];
    this.activeFieldCellKeys = [];
    this.activeShipCellKeys = [];
    this.worldSyncTimerMs = 0;
    this.lastWorldSyncCellKey = "";
    this.radarRefreshTimerMs = 0;
    this.collisionUpdateTimerMs = 0;
    this.cachedRadarSources = [];
    ...
    this.galaxyDefinition.zones.forEach((zone) => {
      this.galaxyZonesById.set(zone.id, zone);
    });
    ...
    this.rebuildMinerAssignmentCache();
    this.cachedRadarSources = this.collectRadarSources();
    this.lastWorldSyncCellKey = getSpaceCellKeyAtPosition(this.shipRoot.x, this.shipRoot.y, SPACE_WORLD_CONFIG);
    this.worldSyncTimerMs = WORLD_SYNC_INTERVAL_MS;
    this.radarRefreshTimerMs = RADAR_REFRESH_INTERVAL_MS;
    this.collisionUpdateTimerMs = 0;
    this.createHud();
    this.createTouchUi();
  ...
  update(_time: number, delta: number): void {
    ...
    this.worldSyncTimerMs = Math.max(0, this.worldSyncTimerMs - delta);
    this.radarRefreshTimerMs = Math.max(0, this.radarRefreshTimerMs - delta);
    this.collisionUpdateTimerMs = Math.max(0, this.collisionUpdateTimerMs - delta);
    this.syncTrackedMissionPlanet();
    ...
    this.rebuildActiveSpatialBuckets();
    if (this.collisionUpdateTimerMs <= 0) {
      this.resolveFactionShipCollisions();
      this.collisionUpdateTimerMs = COLLISION_UPDATE_INTERVAL_MS;
    }
  ...
  private shouldSyncActiveWorld(): boolean {
    const currentCellKey = getSpaceCellKeyAtPosition(this.shipRoot.x, this.shipRoot.y, SPACE_WORLD_CONFIG);
    const intervalMs = this.hyperdrive.state === "active"
      ? WORLD_SYNC_HYPERDRIVE_INTERVAL_MS
      : WORLD_SYNC_INTERVAL_MS;
    if (this.lastWorldSyncCellKey !== currentCellKey || this.worldSyncTimerMs <= 0) {
      this.lastWorldSyncCellKey = currentCellKey;
      this.worldSyncTimerMs = intervalMs;
      return true;
    }
    return false;
  }
  ...
  private initializeWorldStates(): void {
    this.fieldStates = new Map<string, SpaceFieldObjectState>(this.worldDefinition.fieldSeeds.map((seed) => ([seed.id, {
      ...
      resourceVisualKind: seed.resourceVisualKind,
      resourceYield: seed.resourceYield,
      respawnDurationMs: seed.respawnDurationMs,
      resourceSystemId: seed.resourceSystemId,
      resourceZoneId: seed.resourceZoneId,
      x: seed.x,
      y: seed.y,
      ...
      destroyed: Boolean(
        seed.resourceType
        && getFactionResourceNodeById(this.forceState, seed.id)?.remainingYield === 0
        && (getFactionResourceNodeById(this.forceState, seed.id)?.depletedUntilSimTimeMs ?? 0) > this.forceState.simulationTimeMs
      ),
    }] as const)));
  ...
  private createShipStateFromSeed(seed: SpaceWorldDefinition["factionSeeds"][number]): SpaceFactionShipState {
    ...
    const forceShip = seed.originPoolId ? getFactionForceShipById(this.forceState, seed.id) : null;
    return {
      id: seed.id,
      cellKey: seed.cellKey,
      ...
      rankLevel,
      rankXp: forceShip?.rankXp ?? 0,
      kills: forceShip?.kills ?? 0,
      ...
      routeWaitRemainingMs: 0,
      supportRepairCooldown: 0,
      cargo: forceShip?.cargo ? { ...forceShip.cargo } : {},
      miningState: forceShip?.minerState ?? (seed.shipRole === "miner-ship" ? "idle" : null),
      targetResourceNodeId: forceShip?.targetResourceNodeId ?? null,
      cargoCarrierShipId: forceShip?.cargoCarrierShipId ?? null,
      miningProgress: forceShip?.miningProgress ?? 0,
      destroyed: false,
    };
  }

  private syncActiveWorld(force = false): void {
    const desiredFieldCellKeys = getSpaceCellKeysWithinDistance(
      this.shipRoot.x,
      this.shipRoot.y,
      ACTIVE_FIELD_STREAM_DISTANCE,
    );
    const desiredShipCellKeys = getSpaceCellKeysWithinDistance(
      this.shipRoot.x,
      this.shipRoot.y,
      ACTIVE_SHIP_STREAM_DISTANCE,
    );
    const desiredCelestialCellKeys = getSpaceCellKeysWithinDistance(
      this.shipRoot.x,
      this.shipRoot.y,
      ACTIVE_CELESTIAL_STREAM_DISTANCE,
    );
  ...
  private syncActiveFieldObjects(cellKeys: SpaceWorldCellKey[], shouldActivate: boolean): void {
    ...
    this.fieldStates.forEach((state) => {
      ...
      const fieldObject = this.createFieldObject(state);
      this.asteroids.push(fieldObject);
  ...
  private syncActiveFactionShips(cellKeys: SpaceWorldCellKey[], shouldActivate: boolean): void {
    ...
    this.shipStates.forEach((state) => {
      ...
      const ship = this.createFactionShip(state);
      this.factionShips.push(ship);
      this.activeFactionShipsById.set(ship.id, ship);
      if (this.shouldSpawnNpcHyperjumpArrival(state)) {
        this.spawnMissionHyperjumpArrival(state.x, state.y, this.getShipPalette(state).glowColor);
      }
    });
  }
  ...
  private mergeForceShipState(
    ...
    return {
      ...
      cargo: { ...desiredState.cargo },
      miningState: desiredState.miningState,
      targetResourceNodeId: desiredState.targetResourceNodeId,
      cargoCarrierShipId: desiredState.cargoCarrierShipId,
      miningProgress: desiredState.miningProgress,
      destroyed: false,
    };
  }
  ...
  private applyActiveForceShipState(
    ...
    ship.cargo = { ...nextState.cargo };
    ship.miningState = nextState.miningState;
    ship.targetResourceNodeId = nextState.targetResourceNodeId;
    ship.cargoCarrierShipId = nextState.cargoCarrierShipId;
    ship.miningProgress = nextState.miningProgress;
    ...
    this.setShipRankBadgeVisibility(ship);
  }
  ...
  private shouldSpawnNpcHyperjumpArrival(state: SpaceFactionShipState): boolean {
    if (!state.originPoolId || !state.originSystemId) {
      return false;
    }
    if (state.slotKind && state.slotKind !== "command" && state.shipRole !== "miner-ship") {
      return false;
    }
    const originSystem = this.galaxySystemsById.get(state.originSystemId);
    if (!originSystem) {
      return false;
    }
    const distanceFromOrigin = Phaser.Math.Distance.Between(state.x, state.y, originSystem.x, originSystem.y);
    return distanceFromOrigin >= 2600;
  }
  ...
  private updateForceProduction(deltaMs: number): void {
    ...
    this.forceStateDirty = true;
    this.syncFieldNodeAvailabilityFromForceState();
  ...
  private captureFieldObjectState(fieldObject: SpaceFieldObject, destroyed = false): SpaceFieldObjectState {
    return {
      ...
      resourceType: fieldObject.resourceType,
      resourceVisualKind: fieldObject.resourceVisualKind,
      resourceYield: fieldObject.resourceYield,
      respawnDurationMs: fieldObject.respawnDurationMs,
      resourceSystemId: fieldObject.resourceSystemId,
      resourceZoneId: fieldObject.resourceZoneId,
  ...
  private deactivateFieldObject(fieldObject: SpaceFieldObject): void {
    this.syncResourceNodeFromFieldObject(fieldObject);
  ...
  private deactivateFactionShip(ship: SpaceFactionShip): void {
    ...
    this.clearShipTargetReferences(ship.id);
    this.activeFactionShipsById.delete(ship.id);
    ship.miningBeam?.destroy();
    ship.root.destroy(true);
  ...
  private setShipRankBadgeVisibility(ship: Pick<SpaceFactionShip, "shipRole" | "rankBadge">): void {
    ship.rankBadge.setVisible(ship.shipRole !== "miner-ship");
  }
  ...
  private createFieldObject(state: SpaceFieldObjectState): SpaceFieldObject {
    ...
    const hullColor = state.kind === "asteroid"
      ? state.resourceVisualKind === "starforged"
        ? (state.isLarge ? 0xa68437 : 0x8f7430)
        : state.resourceVisualKind === "aetherium"
          ? (state.isLarge ? 0x5478a8 : 0x4b6a97)
          : state.resourceVisualKind === "scrap"
            ? (state.isLarge ? 0x6c6360 : 0x5d5552)
            : state.isLarge
              ? 0x71849f
              : isBelt
                ? 0x7b8698
    ...
    const shadow = this.add.ellipse(0, baseRadius * 0.08, baseRadius * 1.76, baseRadius * 1.2, 0x000000, state.isLarge ? 0.18 : 0.13);
    const hull = this.add.polygon(0, 0, rockPoints, hullColor, 0.98).setStrokeStyle(state.isLarge ? 3 : 2, strokeColor, 0.66);
    const craterA = this.add.circle(-baseRadius * 0.18, baseRadius * 0.14, Math.max(4, baseRadius * 0.16), state.resourceVisualKind === "aetherium" ? 0x5db8ff : state.resourceVisualKind === "starforged" ? 0xdcb566 : 0x34404d, state.resourceVisualKind ? 0.3 : 0.24);
    const craterB = this.add.circle(baseRadius * 0.2, -baseRadius * 0.1, Math.max(3, baseRadius * 0.12), state.resourceVisualKind === "scrap" ? 0x6f6354 : 0x2f3945, 0.18);
    const craterC = state.isLarge
      ? this.add.circle(baseRadius * 0.06, baseRadius * 0.28, Math.max(5, baseRadius * 0.14), state.resourceVisualKind === "starforged" ? 0x7b6128 : 0x26313d, 0.16)
      : null;
    const shardGlow = state.isLarge
      ? this.add.circle(0, 0, baseRadius * 1.08, state.resourceVisualKind === "starforged" ? 0xffd781 : state.resourceVisualKind === "aetherium" ? 0x7ad5ff : 0xcde3ff, state.resourceVisualKind ? 0.14 : 0.08)
      : this.add.circle(0, 0, baseRadius * 0.92, strokeColor, 0.05);
    const vein = state.kind === "asteroid" && state.resourceVisualKind
      ? this.add.line(0, 0, -baseRadius * 0.42, 0, baseRadius * 0.46, baseRadius * 0.24, strokeColor, 0.46)
        .setLineWidth(1, 3)
        .setRotation(hashTextSeed(`${state.id}:vein`) % 628 / 100)
      : null;
    const damageRing = this.add.circle(0, 0, baseRadius * 0.76, 0xff8f57, 0).setStrokeStyle(2, 0xffcc92, 0);
    const visualChildren: Phaser.GameObjects.GameObject[] = [shadow, shardGlow, hull, craterA, craterB];
    if (vein) {
      visualChildren.push(vein);
    }
    if (craterC) {
      visualChildren.push(craterC);
    }
    visualChildren.push(damageRing);
    ...
    return {
      ...
      resourceVisualKind: state.resourceVisualKind,
      resourceYield: state.resourceYield,
      respawnDurationMs: state.respawnDurationMs,
      resourceSystemId: state.resourceSystemId,
      resourceZoneId: state.resourceZoneId,
    };
  }
  ...
  private createFactionShip(state: SpaceFactionShipState): SpaceFactionShip {
    ...
    if (state.shipRole) {
      if (state.shipRole === "miner-ship") {
        children.push(
          this.add.rectangle(-bodyRadius * 0.94, bodyRadius * 0.26, bodyRadius * 0.34, bodyRadius * 0.92, 0x4d5f6c, 0.98).setStrokeStyle(1, palette.trimColor, 0.28),
          this.add.rectangle(bodyRadius * 0.94, bodyRadius * 0.26, bodyRadius * 0.34, bodyRadius * 0.92, 0x4d5f6c, 0.98).setStrokeStyle(1, palette.trimColor, 0.28),
          this.add.rectangle(0, bodyRadius * 0.28, bodyRadius * 0.96, bodyRadius * 1.44, palette.color, 0.96).setStrokeStyle(2, palette.trimColor, 0.72),
          this.add.rectangle(0, -bodyRadius * 0.36, bodyRadius * 0.28, bodyRadius * 0.94, palette.trimColor, 0.88).setStrokeStyle(1, 0xf7fbff, 0.24),
          this.add.rectangle(-bodyRadius * 1.12, -bodyRadius * 0.12, bodyRadius * 0.2, bodyRadius * 0.76, 0x90caff, 0.9).setStrokeStyle(1, 0xf7fbff, 0.26),
          this.add.rectangle(bodyRadius * 1.12, -bodyRadius * 0.12, bodyRadius * 0.2, bodyRadius * 0.76, 0x90caff, 0.9).setStrokeStyle(1, 0xf7fbff, 0.26),
          this.add.rectangle(0, bodyRadius * 1.04, bodyRadius * 0.68, bodyRadius * 0.3, 0x1d3143, 0.96).setStrokeStyle(1, palette.trimColor, 0.3),
        );
      } else if (state.shipRole === "base-fighter" || state.shipRole === "support-fighter") {
        children.push(
    ...
    const miningBeam = state.shipRole === "miner-ship"
      ? this.add.line(0, 0, 0, 0, 0, 0, palette.glowColor, 0.94)
        .setLineWidth(4, 8)
        .setDepth(14)
    ...
    const ship: SpaceFactionShip = {
      ...
      cargo: { ...state.cargo },
      miningState: state.miningState ?? null,
      targetResourceNodeId: state.targetResourceNodeId ?? null,
      cargoCarrierShipId: state.cargoCarrierShipId ?? null,
      miningProgress: state.miningProgress ?? 0,
      miningBeam,
    };
    this.setShipRankBadgeVisibility(ship);
    return ship;
  ...
  private getShipCombatConfig(
    ...
    const effectiveRankLevel = ship.shipRole === "miner-ship" ? 1 : (ship.rankLevel ?? 1);
    const speedMultiplier = getFactionShipRankSpeedMultiplier(effectiveRankLevel);
    const hullMultiplier = getFactionShipRankHullMultiplier(effectiveRankLevel);
    return {
      ...baseFaction,
      ...
      fireRange: baseFaction.fireRange * roleProfile.fireRangeMultiplier * (1 + ((effectiveRankLevel - 1) * 0.04)),
  ...
  private isShipTravelingToAssignment(ship: Pick<SpaceFactionShip, "originPoolId" | "assignmentZoneId" | "originZoneId" | "root" | "originSystemId" | "originRaceId">): boolean {
    ...
    const assignedZone = this.galaxyZonesById.get(ship.assignmentZoneId) ?? null;
  ...
  private trySupportRepair(ship: SpaceFactionShip): void {
    ...
    const repairTarget = this.getNearbyFactionShips(ship.root.x, ship.root.y, roleProfile.supportRepairRange + 60)
      .filter((ally) => {
  ...
  private updateFieldObjects(dt: number): void {
    for (const fieldObject of this.asteroids) {
      ...
      this.syncFieldStateFromFieldObject(fieldObject);
      this.syncResourceNodeFromFieldObject(fieldObject);
  ...
  private getFieldCollisionRadius(fieldObject: Pick<SpaceFieldObjectState, "kind" | "baseRadius" | "radius">): number {
    if (fieldObject.kind === "debris") {
      return Math.max(8, fieldObject.radius * 0.52);
    }
    return Math.max(12, fieldObject.radius * 0.68);
  }
  ...
  private rebuildActiveSpatialBuckets(): void {
    ...
    this.asteroids.forEach((fieldObject) => {
      ...
      if (bucket) {
        bucket.push(fieldObject);
      } else {
        this.activeAsteroidBuckets.set(bucketKey, [fieldObject]);
      }
    });

    this.factionShips.forEach((ship) => {
      this.activeFactionShipsById.set(ship.id, ship);
      const bucketKey = getActiveSpatialBucketKey(ship.root.x, ship.root.y);
      const bucket = this.activeShipBuckets.get(bucketKey);
      if (bucket) {
        bucket.push(ship);
      } else {
        this.activeShipBuckets.set(bucketKey, [ship]);
      }
    });
  }

  private syncFieldStateFromFieldObject(fieldObject: SpaceFieldObject): void {
    const fieldState = this.fieldStates.get(fieldObject.id);
    if (!fieldState) {
      return;
    }
    fieldState.cellKey = getSpaceCellKeyAtPosition(fieldObject.root.x, fieldObject.root.y, SPACE_WORLD_CONFIG);
    fieldState.x = fieldObject.root.x;
    fieldState.y = fieldObject.root.y;
    fieldState.radius = fieldObject.radius;
    fieldState.hp = Math.max(0, fieldObject.hp);
    fieldState.velocityX = fieldObject.velocity.x;
    fieldState.velocityY = fieldObject.velocity.y;
    fieldState.rotation = fieldObject.root.rotation;
    fieldState.flash = fieldObject.flash;
    fieldState.destroyed = false;
  }

  private rebuildMinerAssignmentCache(): void {
    this.minerAssignedShipIdsByNodeId.clear();
    this.forceState.pools.forEach((pool) => {
      pool.activeShips.forEach((ship) => {
        if (ship.role !== "miner-ship" || !ship.targetResourceNodeId) {
          return;
        }
        const bucket = this.minerAssignedShipIdsByNodeId.get(ship.targetResourceNodeId);
        if (bucket) {
          bucket.push(ship.id);
        } else {
          this.minerAssignedShipIdsByNodeId.set(ship.targetResourceNodeId, [ship.id]);
        }
      });
    });
    this.minerAssignedShipIdsByNodeId.forEach((shipIds) => {
      shipIds.sort();
    });
  }

  private getNearbyFieldObjects(x: number, y: number, radius: number): SpaceFieldObject[] {
    const results: SpaceFieldObject[] = [];
    const seen = new Set<string>();
    getActiveSpatialBucketKeysAround(x, y, radius).forEach((bucketKey) => {
      (this.activeAsteroidBuckets.get(bucketKey) ?? []).forEach((fieldObject) => {
        if (seen.has(fieldObject.id)) {
          return;
        }
        seen.add(fieldObject.id);
        results.push(fieldObject);
      });
    });
    return results;
  }

  private getNearbyFactionShips(x: number, y: number, radius: number): SpaceFactionShip[] {
    const results: SpaceFactionShip[] = [];
    const seen = new Set<string>();
    getActiveSpatialBucketKeysAround(x, y, radius).forEach((bucketKey) => {
      (this.activeShipBuckets.get(bucketKey) ?? []).forEach((ship) => {
        if (seen.has(ship.id)) {
          return;
        }
        seen.add(ship.id);
  ...
  private resolvePlayerAgainstFieldObject(fieldObject: SpaceFieldObject): void {
    ...
    const minimumDistance = this.getFieldCollisionRadius(fieldObject) + PLAYER_RADIUS;
  ...
  private syncFieldNodeAvailabilityFromForceState(): void {
    this.fieldStates.forEach((state, fieldId) => {
      if (!state.resourceType) {
        return;
      }
      const node = getFactionResourceNodeById(this.forceState, fieldId);
      const shouldBeDestroyed = Boolean(
        node
        && node.remainingYield <= 0
        && (node.depletedUntilSimTimeMs ?? 0) > this.forceState.simulationTimeMs,
      );
      if (shouldBeDestroyed === state.destroyed) {
        return;
      }

      if (shouldBeDestroyed) {
        const activeFieldObject = this.getActiveFieldObjectById(fieldId);
      ...
      const seed = this.worldDefinition.fieldSeedIndex[fieldId];
      if (!seed) {
        return;
      }
      const restoredState: SpaceFieldObjectState = {
        id: seed.id,
        cellKey: seed.cellKey,
        kind: seed.kind,
        placementType: seed.placementType,
      ...
      this.fieldStates.set(fieldId, restoredState);
      if (this.activeFieldCellKeys.includes(restoredState.cellKey) && !this.asteroids.some((candidate) => candidate.id === fieldId)) {
        this.asteroids.push(this.createFieldObject(restoredState));
      }
    });
  }

  private hideMiningBeam(ship: SpaceFactionShip): void {
    if (!ship.miningBeam) {
      return;
    }
  ...
  private isWithinPlayerDetailRange(x: number, y: number, radius: number): boolean {
    const dx = x - this.shipRoot.x;
    const dy = y - this.shipRoot.y;
    return ((dx * dx) + (dy * dy)) <= (radius * radius);
  }

  private syncResourceNodeFromFieldObject(fieldObject: Pick<SpaceFieldObject, "id" | "resourceType" | "root">): void {
    if (!fieldObject.resourceType) {
      return;
    }
    const node = this.resourceNodesByFieldId.get(fieldObject.id);
    if (!node) {
      return;
    }
    node.x = fieldObject.root.x;
    node.y = fieldObject.root.y;
    node.cellKey = getSpaceCellKeyAtPosition(fieldObject.root.x, fieldObject.root.y, SPACE_WORLD_CONFIG);
  }

  private getActiveFieldObjectById(fieldId: string): SpaceFieldObject | null {
    return this.activeFieldObjectsById.get(fieldId)
      ?? this.asteroids.find((fieldObject) => fieldObject.id === fieldId)
      ?? null;
  }
  ...
  private getMinerNodePriorityScore(
    ...
    context: {
      ...
      requireActiveFieldObject: boolean;
    },
  ): number {
    ...
    const controllerId = node.zoneId
      ? this.galaxyZonesById.get(node.zoneId)?.currentControllerId ?? null
      : null;
    ...
    const localToOrigin = distanceFromOrigin <= 1200;
    const ironStock = context.stockpile["iron-ore"] ?? 0;
    const ironNeed = Math.max(0, MINER_IRON_PRIORITY_STOCKPILE_THRESHOLD - ironStock);
    const assignedMiners = (this.minerAssignedShipIdsByNodeId.get(node.id) ?? []).filter((shipId) => shipId !== context.shipId).length;
    const slotCount = getFactionResourceNodeMiningSlotCount(node);

    let score = 0;
    score += sameSystem ? 420 : sameSector ? 120 : 18;
    score += localToOrigin ? 160 : 0;
    score += friendly ? 64 : 0;
    score += node.isLarge ? 28 : 0;
    score += Math.min(52, node.remainingYield * 0.85);
    score += node.placementType === "belt" ? 18 : node.placementType === "cluster" ? 9 : 0;
    score += Math.max(0, slotCount - assignedMiners) * 24;
    score -= Math.max(0, assignedMiners - slotCount + 1) * 78;

    switch (node.resourceType) {
      case "iron-ore":
        score += ironNeed > 0 ? 260 + Math.min(96, ironNeed * 1.2) : 84;
        break;
    ...
    if (context.requireActiveFieldObject) {
      score += anchor.activeFieldObject ? 240 : -320;
    }

    score -= distanceFromShip * 0.085;
    score -= distanceFromOrigin * 0.065;
    if (!sameSystem) {
      score -= 42;
    }
    if (!sameSector) {
  ...
  private findNearestMinerThreat(ship: SpaceFactionShip): SpaceFactionShip | null {
    ...
    this.getNearbyFactionShips(ship.root.x, ship.root.y, MINER_THREAT_AVOID_RANGE + 80).forEach((candidate) => {
      if (candidate.id === ship.id || !this.isShipHostileToShip(candidate, ship)) {
        return;
      }
      const dx = candidate.root.x - ship.root.x;
      const dy = candidate.root.y - ship.root.y;
      const distanceSq = (dx * dx) + (dy * dy);
      if (distanceSq > MINER_THREAT_AVOID_RANGE * MINER_THREAT_AVOID_RANGE || distanceSq >= nearestDistanceSq) {
        return;
      }
  ...
  private pickBestMinerResourceNode(
    ...
    positionX: number,
    positionY: number,
    requireActiveFieldObject = false,
  ) {
    ...
    const candidateNodes = this.forceState.resourceNodes.filter((node) => {
      ...
      if (requireActiveFieldObject && !anchor.activeFieldObject) {
        return false;
      }
      if (node.systemId === originSystemId) {
        return true;
      }
      if (originSectorId && node.sectorId === originSectorId) {
        return true;
      }
      if (!originSystem) {
        return false;
    ...
    const ironNeed = Math.max(0, MINER_IRON_PRIORITY_STOCKPILE_THRESHOLD - (stockpile["iron-ore"] ?? 0));
    const localIronCandidates = ironNeed > 0
      ? candidateNodes.filter((node) => node.systemId === originSystemId && node.resourceType === "iron-ore")
      : [];
    const nodesToScore = localIronCandidates.length > 0 ? localIronCandidates : candidateNodes;

    const scoredNodes = nodesToScore
      .map((node) => ({
        node,
  ...
  private getMinerNodeSlotTarget(node: FactionResourceNodeRecord, shipId: string): Phaser.Math.Vector2 {
    const assignedMiners = [...(this.minerAssignedShipIdsByNodeId.get(node.id) ?? [])];
    const slotCount = Math.max(1, getFactionResourceNodeMiningSlotCount(node));
    const slotIndex = Math.max(0, (() => {
      const existingIndex = assignedMiners.indexOf(shipId);
      if (existingIndex >= 0) {
        return existingIndex;
      }
      assignedMiners.push(shipId);
      assignedMiners.sort();
      return assignedMiners.indexOf(shipId);
    })());
    const anchor = this.getResourceNodeAnchorMetrics(node);
  ...
  private depositMinerCargoToSystem(ship: SpaceFactionShip): boolean {
    const forceShip = getFactionForceShipById(this.forceState, ship.id);
    if (!ship.originSystemId || !forceShip || getFactionShipCargoAmount(forceShip) <= 0) {
      return false;
    }
    const cargo = clearFactionShipCargo(forceShip);
    const deposited = depositFactionResourcesToSystem(this.forceState, ship.originSystemId, cargo);
    ship.cargo = { ...forceShip.cargo };
    if (deposited) {
      this.forceStateDirty = true;
    }
    return deposited;
  ...
  private harvestMinerResourceNode(ship: SpaceFactionShip, nodeId: string, requestedAmount: number): number {
    const forceShip = getFactionForceShipById(this.forceState, ship.id);
    const node = getFactionResourceNodeById(this.forceState, nodeId);
    if (!forceShip || !node || node.remainingYield <= 0) {
      return 0;
    }
    const added = addFactionShipCargo(
      forceShip,
      node.resourceType,
      Math.min(
        Math.max(1, Math.floor(requestedAmount)),
        Math.max(0, Math.floor(node.remainingYield)),
      ),
    );
    if (added <= 0) {
      ship.miningState = "returning";
      ship.targetResourceNodeId = nodeId;
      return 0;
    }
    const extraction = extractFactionResourceNodeYield(this.forceState, node.id, added);
    ship.cargo = { ...forceShip.cargo };
    this.forceStateDirty = true;
    if (extraction.depleted) {
      const activeFieldObject = this.getActiveFieldObjectById(node.fieldId);
      if (activeFieldObject) {
        this.breakFieldObject(activeFieldObject, false);
      } else {
        const fieldState = this.fieldStates.get(node.fieldId);
        if (fieldState) {
          fieldState.destroyed = true;
        }
      }
      this.syncFieldNodeAvailabilityFromForceState();
    }
    return extraction.extractedAmount;
  }

  private findEligibleMinerCargoCarrier(ship: SpaceFactionShip): SpaceFactionShip | null {
    return this.getNearbyFactionShips(ship.root.x, ship.root.y, MINER_DEPOSIT_RANGE + 90).find((candidate) => candidate.id !== ship.id
      && candidate.factionId === ship.factionId
      && candidate.originSystemId === ship.originSystemId
      && (candidate.shipRole === "attack-warship" || candidate.shipRole === "defense-warship")) ?? null;
  }

  private depositMinerCargoToCarrier(miner: SpaceFactionShip, carrier: SpaceFactionShip): boolean {
    const minerForceShip = getFactionForceShipById(this.forceState, miner.id);
    const carrierForceShip = getFactionForceShipById(this.forceState, carrier.id);
    if (!minerForceShip || !carrierForceShip || getFactionShipCargoAmount(minerForceShip) <= 0) {
      return false;
    }

    let transferred = false;
    Object.entries({ ...minerForceShip.cargo }).forEach(([resourceType, amount]) => {
      const added = addFactionShipCargo(carrierForceShip, resourceType as FactionResourceType, amount ?? 0);
      if (added <= 0) {
        return;
      }
      const remaining = Math.max(0, (minerForceShip.cargo[resourceType as FactionResourceType] ?? 0) - added);
      if (remaining > 0) {
        minerForceShip.cargo[resourceType as FactionResourceType] = remaining;
      } else {
        delete minerForceShip.cargo[resourceType as FactionResourceType];
      }
      transferred = true;
    });

    miner.cargo = { ...minerForceShip.cargo };
    carrier.cargo = { ...carrierForceShip.cargo };
    if (transferred) {
      this.forceStateDirty = true;
    }
    return transferred;
  ...
  private getCargoCarrierReturnMovement(ship: SpaceFactionShip): Phaser.Math.Vector2 | null {
    const forceShip = getFactionForceShipById(this.forceState, ship.id);
    if (!forceShip || !ship.originSystemId || getFactionShipCargoAmount(forceShip) <= 0) {
      return null;
    }
    const originSystem = this.galaxySystemsById.get(ship.originSystemId);
    if (!originSystem) {
      return null;
    }
    const distanceToOrigin = Phaser.Math.Distance.Between(ship.root.x, ship.root.y, originSystem.x, originSystem.y);
    if (distanceToOrigin <= MINER_DEPOSIT_RANGE) {
      const cargo = clearFactionShipCargo(forceShip);
      if (depositFactionResourcesToSystem(this.forceState, ship.originSystemId, cargo)) {
        this.forceStateDirty = true;
      }
      ship.cargo = { ...forceShip.cargo };
      return new Phaser.Math.Vector2();
    }
    return new Phaser.Math.Vector2(originSystem.x - ship.root.x, originSystem.y - ship.root.y).normalize();
  }

  private harvestMinerResourceNodeState(
    forceShip: ReturnType<typeof getFactionForceShipById>,
    shipState: SpaceFactionShipState,
    nodeId: string,
    requestedAmount: number,
  ): { harvested: number; depleted: boolean } {
    if (!forceShip) {
      return { harvested: 0, depleted: false };
    }
    const node = getFactionResourceNodeById(this.forceState, nodeId);
    if (!node || node.remainingYield <= 0) {
      return { harvested: 0, depleted: false };
    }
    const harvested = addFactionShipCargo(
      forceShip,
      node.resourceType,
      Math.min(Math.max(1, Math.floor(requestedAmount)), Math.max(0, Math.floor(node.remainingYield))),
    );
    if (harvested <= 0) {
      return { harvested: 0, depleted: false };
    }
    shipState.cargo = { ...forceShip.cargo };
    const extraction = extractFactionResourceNodeYield(this.forceState, node.id, harvested);
    return {
      harvested: extraction.extractedAmount,
      depleted: extraction.depleted,
    };
  }

  private moveInactiveShipStateToward(
    shipState: SpaceFactionShipState,
    targetX: number,
    targetY: number,
    dt: number,
  ): void {
    const faction = this.getShipCombatConfig(shipState);
    const dx = targetX - shipState.x;
    const dy = targetY - shipState.y;
    const distance = Math.hypot(dx, dy);
    if (distance <= 0.001) {
      shipState.velocityX *= Math.max(0, 1 - (1.9 * dt));
      shipState.velocityY *= Math.max(0, 1 - (1.9 * dt));
      return;
    }

    const directionX = dx / distance;
    const directionY = dy / distance;
    shipState.velocityX += directionX * faction.acceleration * dt;
    shipState.velocityY += directionY * faction.acceleration * dt;
    const speed = Math.hypot(shipState.velocityX, shipState.velocityY);
    if (speed > faction.maxSpeed) {
      const scale = faction.maxSpeed / speed;
      shipState.velocityX *= scale;
      shipState.velocityY *= scale;
    }
    shipState.x += shipState.velocityX * dt;
    shipState.y += shipState.velocityY * dt;
    shipState.rotation = Math.atan2(directionY, directionX) + Math.PI * 0.5;
    shipState.aimX = directionX;
    shipState.aimY = directionY;
    shipState.patrolX = targetX;
    shipState.patrolY = targetY;
    shipState.cellKey = getSpaceCellKeyAtPosition(shipState.x, shipState.y, SPACE_WORLD_CONFIG);
  }

  private updateInactiveMinerShips(dt: number): void {
    const activeIds = new Set(this.factionShips.map((ship) => ship.id));
    let changed = false;
    let depletedNodeChanged = false;

    for (const pool of this.forceState.pools) {
      for (const forceShip of pool.activeShips) {
        if (forceShip.role !== "miner-ship" || activeIds.has(forceShip.id) || !pool.originSystemId) {
          continue;
        }

        const shipState = this.shipStates.get(forceShip.id);
        const originSystem = this.galaxySystemsById.get(pool.originSystemId);
        if (!shipState || !originSystem) {
          continue;
        }

        shipState.cargo = { ...forceShip.cargo };
        let node = forceShip.targetResourceNodeId ? getFactionResourceNodeById(this.forceState, forceShip.targetResourceNodeId) : null;
        if (!node || node.remainingYield <= 0 || node.depletedUntilSimTimeMs !== null) {
          node = this.pickBestMinerResourceNode(shipState.id, pool.originSystemId, shipState.originRaceId, shipState.x, shipState.y);
          forceShip.targetResourceNodeId = node?.id ?? null;
          shipState.targetResourceNodeId = node?.id ?? null;
        }

        const cargoAmount = getFactionShipCargoAmount(forceShip);
        if (cargoAmount >= MINER_RETURN_CARGO_THRESHOLD || !node) {
          forceShip.minerState = cargoAmount > 0 ? "returning" : "idle";
          shipState.miningState = forceShip.minerState;
          if (cargoAmount > 0 && Phaser.Math.Distance.Between(shipState.x, shipState.y, originSystem.x, originSystem.y) <= MINER_DEPOSIT_RANGE) {
            const cargo = clearFactionShipCargo(forceShip);
            if (depositFactionResourcesToSystem(this.forceState, pool.originSystemId, cargo)) {
              changed = true;
            }
            shipState.cargo = { ...forceShip.cargo };
            forceShip.targetResourceNodeId = null;
            shipState.targetResourceNodeId = null;
          } else {
            this.moveInactiveShipStateToward(shipState, originSystem.x, originSystem.y, dt);
          }
          shipState.miningProgress = 0;
          forceShip.miningProgress = 0;
          changed = true;
          continue;
        }

        const slotTarget = this.getMinerNodeSlotTarget(node, forceShip.id);
        const distanceToNode = Phaser.Math.Distance.Between(shipState.x, shipState.y, slotTarget.x, slotTarget.y);
        if (distanceToNode <= MINER_MINING_RANGE) {
          forceShip.minerState = "mining";
          shipState.miningState = "mining";
          forceShip.miningProgress += dt * MINER_MINING_RATE_PER_SEC;
          const requestedAmount = Math.floor(forceShip.miningProgress);
          if (requestedAmount > 0) {
            const { harvested, depleted } = this.harvestMinerResourceNodeState(forceShip, shipState, node.id, requestedAmount);
            if (harvested > 0) {
              forceShip.miningProgress = Math.max(0, forceShip.miningProgress - harvested);
              changed = true;
            }
            if (depleted) {
              depletedNodeChanged = true;
              forceShip.targetResourceNodeId = null;
              shipState.targetResourceNodeId = null;
            }
          }
        } else {
          forceShip.minerState = "travel-to-node";
          shipState.miningState = "travel-to-node";
          this.moveInactiveShipStateToward(shipState, slotTarget.x, slotTarget.y, dt);
          forceShip.miningProgress = 0;
        }

        shipState.cargoCarrierShipId = forceShip.cargoCarrierShipId;
        shipState.cargo = { ...forceShip.cargo };
        shipState.miningProgress = forceShip.miningProgress;
        changed = true;
      }
    }

    if (depletedNodeChanged) {
      this.syncFieldNodeAvailabilityFromForceState();
    }
    if (changed) {
      this.forceStateDirty = true;
  ...
  private updateMinerShip(
    ship: SpaceFactionShip,
    dt: number,
    faction: (typeof SPACE_FACTIONS)[SpaceFactionId],
    palette: { color: number; trimColor: number; glowColor: number },
  ): void {
    const forceShip = getFactionForceShipById(this.forceState, ship.id);
    if (!forceShip || !ship.originSystemId) {
      this.hideMiningBeam(ship);
      return;
    }

    ship.cargo = { ...forceShip.cargo };
    ship.cargoCarrierShipId = null;
    const originSystem = this.galaxySystemsById.get(ship.originSystemId);
    if (!originSystem) {
      this.hideMiningBeam(ship);
      return;
    ...
    const threat = this.findNearestMinerThreat(ship);
    let movement = new Phaser.Math.Vector2();
    let aimDirection = ship.aimDirection.clone();
    if (threat) {
      ship.miningState = "fleeing";
      this.hideMiningBeam(ship);
      movement = this.getFleeMovement(ship, new Phaser.Math.Vector2(threat.root.x, threat.root.y));
      ship.patrolTarget.set(originSystem.x, originSystem.y);
    } else {
      const cargoAmount = getFactionShipCargoAmount(forceShip);
      let node = ship.targetResourceNodeId ? getFactionResourceNodeById(this.forceState, ship.targetResourceNodeId) : null;
      if (!node || node.remainingYield <= 0 || node.depletedUntilSimTimeMs !== null) {
        node = this.pickBestMinerResourceNode(ship.id, ship.originSystemId, ship.originRaceId, ship.root.x, ship.root.y, true);
        ship.targetResourceNodeId = node?.id ?? null;
      }
      const nodeAnchor = node ? this.getResourceNodeAnchorMetrics(node) : null;
      if (node && (!nodeAnchor || !nodeAnchor.activeFieldObject)) {
      ...
      if (cargoAmount >= MINER_RETURN_CARGO_THRESHOLD || cargoAmount >= MINER_CARGO_CAPACITY || !node) {
        ...
        ship.patrolTarget.set(originSystem.x, originSystem.y);
        const distanceToOrigin = Phaser.Math.Distance.Between(ship.root.x, ship.root.y, originSystem.x, originSystem.y);
        const carrier = cargoAmount > 0 ? this.findEligibleMinerCargoCarrier(ship) : null;
        if (carrier && this.depositMinerCargoToCarrier(ship, carrier)) {
          ship.miningState = "idle";
          ship.targetResourceNodeId = null;
          ship.cargoCarrierShipId = carrier.id;
          forceShip.targetResourceNodeId = null;
          forceShip.cargoCarrierShipId = carrier.id;
        } else if (distanceToOrigin <= MINER_DEPOSIT_RANGE && cargoAmount > 0) {
          this.depositMinerCargoToSystem(ship);
          ship.miningState = "idle";
          ship.targetResourceNodeId = null;
          forceShip.targetResourceNodeId = null;
        } else if (distanceToOrigin > 0.001) {
          movement = new Phaser.Math.Vector2(originSystem.x - ship.root.x, originSystem.y - ship.root.y).normalize();
        }
        this.hideMiningBeam(ship);
      } else {
        ...
        const asteroidMiningRange = (nodeAnchor?.collisionRadius ?? 24) + (MINER_MINING_RANGE * 0.68);
        if (
          distanceToSlot <= MINER_MINING_RANGE
          && distanceToAsteroid <= asteroidMiningRange
          && canMineVisibleAsteroid
          && nodeAnchor
        ) {
          ship.miningState = "mining";
          ship.miningProgress += dt * MINER_MINING_RATE_PER_SEC;
          if (ship.miningBeam) {
            ship.miningBeam.setPosition(0, 0);
            ship.miningBeam.setTo(ship.root.x, ship.root.y, nodeAnchor.x, nodeAnchor.y);
            ship.miningBeam.setLineWidth(4, 10);
            ship.miningBeam.setStrokeStyle(4, palette.glowColor, 0.96);
            ship.miningBeam.setVisible(true);
          }
          const requestedAmount = Math.floor(ship.miningProgress);
          if (requestedAmount > 0) {
            const harvested = this.harvestMinerResourceNode(ship, node.id, Math.min(requestedAmount, MINER_CARGO_CAPACITY - cargoAmount));
            if (harvested > 0) {
              ship.miningProgress = Math.max(0, ship.miningProgress - harvested);
            }
            if (harvested <= 0 || getFactionShipCargoAmount(forceShip) >= MINER_CARGO_CAPACITY) {
              ship.miningState = "returning";
            }
          }
          ...
          if (
            distanceToSlot > MINER_MINING_RANGE * 0.6
            || distanceToAsteroid > (nodeAnchor?.collisionRadius ?? 24) + MINER_MINING_RANGE
          ) {
    ...
    if (ship.miningState !== "mining") {
      ship.miningProgress = 0;
    }
    this.constrainMovingBody(ship.root, ship.radius, ship.velocity, 0.22);

    ship.damageRing.setStrokeStyle(2, palette.trimColor, ship.flash * 0.72);
    ship.damageRing.setFillStyle(palette.glowColor, ship.flash * 0.14);
    const velocityPulse = Phaser.Math.Clamp(ship.velocity.length() / Math.max(1, faction.maxSpeed), 0, 1);
    ship.thruster.setFillStyle(palette.glowColor, 0.14 + (velocityPulse * 0.1));
    ship.thruster.setScale(1, movement.lengthSq() > 0 ? 0.95 + (velocityPulse * 0.22) : 0.78);

    forceShip.cargo = { ...ship.cargo };
    forceShip.minerState = ship.miningState;
    forceShip.targetResourceNodeId = ship.targetResourceNodeId;
    forceShip.cargoCarrierShipId = ship.cargoCarrierShipId;
    forceShip.miningProgress = ship.miningProgress;
  }
  ...
  private updateFactionShips(dt: number): void {
    for (const ship of this.factionShips) {
      ...
      if (ship.shipRole === "miner-ship") {
        this.updateMinerShip(ship, dt, faction, palette);
        continue;
      }

      const shouldRunCombatLogic = this.isWithinPlayerDetailRange(ship.root.x, ship.root.y, SPACE_COMBAT_DETAIL_RADIUS)
        || ship.provokedByPlayer
        || ship.provokedByShips.size > 0;
      const target = shouldRunCombatLogic ? this.selectShipTarget(ship) : null;
      const fleeTarget = target ? null : this.findNearestSmugglerThreat(ship);
      const carrierReturnMovement = target ? null : this.getCargoCarrierReturnMovement(ship);
      ...
      } else if (carrierReturnMovement) {
        movement.copy(carrierReturnMovement);
        if (movement.lengthSq() > 0) {
          aimDirection = movement.clone().normalize();
        }
  ...
  private selectShipTarget(ship: SpaceFactionShip): { kind: "player" } | { kind: "ship"; ship: SpaceFactionShip } | null {
    ...
    this.getNearbyFactionShips(ship.root.x, ship.root.y, faction.detectRange + 120).forEach((targetShip) => {
  ...
  private findNearestSmugglerThreat(ship: SpaceFactionShip): Phaser.Math.Vector2 | null {
    ...
    this.getNearbyFactionShips(ship.root.x, ship.root.y, 600).forEach((otherShip) => {
  ...
  private getAsteroidAvoidanceDirection(ship: SpaceFactionShip, desiredDirection: Phaser.Math.Vector2): Phaser.Math.Vector2 {
    const nearbyFields = this.getNearbyFieldObjects(
      ship.root.x,
      ship.root.y,
      ASTEROID_AVOIDANCE_RADIUS + ship.radius + 220,
    );
    const desired = desiredDirection.lengthSq() > 0.001 ? desiredDirection.clone().normalize() : null;
    const avoidance = new Phaser.Math.Vector2();
    nearbyFields.forEach((fieldObject) => {
      const dx = fieldObject.root.x - ship.root.x;
      const dy = fieldObject.root.y - ship.root.y;
      const distance = Math.max(0.001, Math.hypot(dx, dy));
      const minDistance = this.getFieldCollisionRadius(fieldObject) + ship.radius + ASTEROID_AVOIDANCE_RADIUS;
      if (distance > minDistance) {
        return;
      }
      const toObstacleX = dx / distance;
      const toObstacleY = dy / distance;
      const aheadness = desired ? Math.max(0, (desired.x * toObstacleX) + (desired.y * toObstacleY)) : 0.42;
    ...
    return avoidance.lengthSq() > 0.001 ? avoidance.normalize() : avoidance;
  }

  private getShipSeparationDirection(ship: SpaceFactionShip): Phaser.Math.Vector2 {
    const nearbyShips = this.getNearbyFactionShips(ship.root.x, ship.root.y, FLEET_AVOIDANCE_RADIUS + ship.radius + 120);
    const separation = new Phaser.Math.Vector2();
    nearbyShips.forEach((candidate) => {
      if (candidate.id === ship.id) {
        return;
      }
      const dx = ship.root.x - candidate.root.x;
      const dy = ship.root.y - candidate.root.y;
      const distance = Math.max(0.001, Math.hypot(dx, dy));
      const minDistance = ship.radius + candidate.radius + FLEET_AVOIDANCE_RADIUS;
      if (distance > minDistance) {
        return;
      }
    ...
    return separation.lengthSq() > 0.001 ? separation.normalize() : separation;
  }
  ...
  private getAssignedZoneForShip(ship: Pick<SpaceFactionShip, "assignmentZoneId" | "originZoneId">) {
    ...
    return zoneId ? this.galaxyZonesById.get(zoneId) ?? null : null;
  }
  ...
  private getModeAdjustedFormationOffset(
    ship: Pick<SpaceFactionShip, "formationOffsetX" | "formationOffsetY" | "fleetMode">,
    leader: Pick<SpaceFactionShip, "fleetMode">,
  ): { x: number; y: number } {
    const mode = ship.fleetMode ?? leader.fleetMode ?? "patrol-group";
    let x = ship.formationOffsetX;
    let y = ship.formationOffsetY;
  ...
  private getPatrolMovement(ship: SpaceFactionShip): Phaser.Math.Vector2 {
    ...
    if (leader && leader.id !== ship.id) {
      ...
      if (slotDistance <= FORMATION_SLOT_LOCK_DISTANCE) {
        if (leader.velocity.lengthSq() > 1) {
          return leader.velocity.clone().normalize();
        }
        if (slotDistance > 6) {
          return slotVector.normalize();
        }
      ...
      if (slotDistance > 0.001) {
        const catchUpBias = Phaser.Math.Clamp(slotDistance / (FORMATION_RECOVERY_DISTANCE * 1.3), 0.34, 1);
        desiredDirection.add(slotVector.normalize().scale(1.12 * catchUpBias));
        if (leaderForward.lengthSq() > 0.001) {
          desiredDirection.add(leaderForward.clone().scale(0.48 * (1 - Math.min(1, catchUpBias * 0.9))));
        }
      }

      if (leader.velocity.lengthSq() > 1) {
        desiredDirection.add(leader.velocity.clone().normalize().scale(0.44));
      }
    ...
    if (assignedAnchor) {
      ...
      return this.getGuardOrbitMovement(ship);
    }
  ...
  private getGuardOrbitMovement(ship: SpaceFactionShip): Phaser.Math.Vector2 {
    if (!ship.guardAnchor || ship.guardRadius <= 0) {
      return new Phaser.Math.Vector2();
    }

    const offset = new Phaser.Math.Vector2(
      ship.root.x - ship.guardAnchor.x,
      ship.root.y - ship.guardAnchor.y,
    );
    if (offset.lengthSq() <= 0.001) {
      const initialAngle = ((hashTextSeed(ship.id) % 628) / 100);
      return new Phaser.Math.Vector2(Math.cos(initialAngle), Math.sin(initialAngle));
    }

    const distance = offset.length();
    const radial = offset.normalize();
    const targetRadius = Phaser.Math.Clamp(ship.guardRadius * 0.72, 150, ship.guardRadius * 0.9);
    const radialError = distance - targetRadius;
    const tangentSign = ship.strafeSign >= 0 ? 1 : -1;
    const tangent = new Phaser.Math.Vector2(-radial.y * tangentSign, radial.x * tangentSign);
    const correctionStrength = Phaser.Math.Clamp(radialError / Math.max(90, ship.guardRadius * 0.5), -0.86, 0.86);
    const movement = tangent.scale(1.08).add(radial.scale(-correctionStrength));
  ...
  private getPatrolLeader(ship: SpaceFactionShip): SpaceFactionShip | null {
    ...
    return this.activeFactionShipsById.get(ship.leaderId)
      ?? this.factionShips.find((candidate) => candidate.id === ship.leaderId)
      ?? null;
  }
  ...
  private getFormationSlotTarget(ship: SpaceFactionShip, leader: SpaceFactionShip): Phaser.Math.Vector2 {
    ...
    const offset = this.getModeAdjustedFormationOffset(ship, leader);
    const lookAheadScale = Phaser.Math.Clamp(leader.velocity.length() / Math.max(1, this.getShipCombatConfig(leader).maxSpeed), 0.04, 0.16);
    const lookAheadX = leader.velocity.x * lookAheadScale;
    const lookAheadY = leader.velocity.y * lookAheadScale;
    return new Phaser.Math.Vector2(
      leader.root.x + (right.x * offset.x) - (forward.x * offset.y) + lookAheadX,
      leader.root.y + (right.y * offset.x) - (forward.y * offset.y) + lookAheadY,
    );
  }
  ...
  private applyFactionMovement(
    ...
    const runLocalAvoidance = ship.shipRole === "miner-ship"
      || this.isWithinPlayerDetailRange(ship.root.x, ship.root.y, SPACE_AVOIDANCE_DETAIL_RADIUS);
    if (runLocalAvoidance) {
      const asteroidAvoidance = this.getAsteroidAvoidanceDirection(ship, desiredDirection);
      if (asteroidAvoidance.lengthSq() > 0.001) {
        steering.add(asteroidAvoidance.scale(ship.shipRole === "miner-ship" ? 1.5 : 1.04));
      }
      const separation = this.getShipSeparationDirection(ship);
      if (separation.lengthSq() > 0.001) {
        steering.add(separation.scale(ship.shipRole === "miner-ship" ? 1.16 : 0.92));
      }
    }

    if (steering.lengthSq() > 0.0001) {
      const normalized = steering.normalize();
    ...
    if (leader && leader.id !== ship.id) {
      const velocityMatchAlpha = Phaser.Math.Clamp(
        dt * (formationSlotDistance > FORMATION_RECOVERY_DISTANCE ? 2.8 : formationSlotDistance <= FORMATION_SLOT_LOCK_DISTANCE ? 7.2 : 4.1),
        0,
        formationSlotDistance <= FORMATION_SLOT_LOCK_DISTANCE ? 0.46 : 0.3,
      );
      ...
      maxSpeed *= formationSlotDistance > FORMATION_RECOVERY_DISTANCE ? 1.12 : formationSlotDistance <= FORMATION_SLOT_LOCK_DISTANCE ? 0.92 : 0.98;
    }
    ...
    if (leader && leader.id !== ship.id && formationSlotDistance <= FORMATION_SLOT_LOCK_DISTANCE) {
      const slotTarget = this.getFormationSlotTarget(ship, leader);
      const settleAlpha = Phaser.Math.Clamp(dt * 5.2, 0, 0.24);
      ship.root.x = Phaser.Math.Linear(ship.root.x, slotTarget.x, settleAlpha);
      ship.root.y = Phaser.Math.Linear(ship.root.y, slotTarget.y, settleAlpha);
    }
  }
  ...
  private resolveFactionShipCollisions(): void {
    this.factionShips.forEach((ship) => {
      if (!this.isWithinPlayerDetailRange(ship.root.x, ship.root.y, SPACE_COLLISION_DETAIL_RADIUS)) {
        return;
      }
      this.getNearbyFieldObjects(ship.root.x, ship.root.y, ship.radius + ASTEROID_AVOIDANCE_RADIUS + 220).forEach((fieldObject) => {
    ...
    this.factionShips.forEach((leftShip) => {
      if (!this.isWithinPlayerDetailRange(leftShip.root.x, leftShip.root.y, SPACE_COLLISION_DETAIL_RADIUS)) {
        return;
      }
      this.getNearbyFactionShips(leftShip.root.x, leftShip.root.y, leftShip.radius + FLEET_AVOIDANCE_RADIUS + 180).forEach((rightShip) => {
        if (rightShip.id === leftShip.id) {
          return;
        }
        if (!this.isWithinPlayerDetailRange(rightShip.root.x, rightShip.root.y, SPACE_COLLISION_DETAIL_RADIUS)) {
          return;
        }
        const pairKey = leftShip.id < rightShip.id
          ? `${leftShip.id}|${rightShip.id}`
          : `${rightShip.id}|${leftShip.id}`;
  ...
  private resolveFactionShipAgainstFieldObject(ship: SpaceFactionShip, fieldObject: SpaceFieldObject): void {
    ...
    const dy = fieldObject.root.y - ship.root.y;
    const distance = Math.sqrt((dx * dx) + (dy * dy)) || 0.0001;
    const minimumDistance = this.getFieldCollisionRadius(fieldObject) + ship.radius;
    if (distance >= minimumDistance) {
      return;
    }
  ...
  private updateProjectiles(dt: number): void {
    for (let shotIndex = this.shots.length - 1; shotIndex >= 0; shotIndex -= 1) {
      ...
      if (shot.ownerKind === "player") {
        const nearbyShips = this.getNearbyFactionShips(shot.sprite.x, shot.sprite.y, 180);
        for (let shipIndex = nearbyShips.length - 1; shipIndex >= 0; shipIndex -= 1) {
          const ship = nearbyShips[shipIndex];
        ...
        const nearbyFieldObjects = this.getNearbyFieldObjects(shot.sprite.x, shot.sprite.y, 180);
        for (let objectIndex = nearbyFieldObjects.length - 1; objectIndex >= 0; objectIndex -= 1) {
          const fieldObject = nearbyFieldObjects[objectIndex];
          const dx = fieldObject.root.x - shot.sprite.x;
          const dy = fieldObject.root.y - shot.sprite.y;
          const combinedRadius = this.getFieldCollisionRadius(fieldObject) + shot.radius;
          if ((dx * dx) + (dy * dy) > combinedRadius * combinedRadius) {
            continue;
          }
  ...
  private breakFieldObject(fieldObject: SpaceFieldObject, spawnPlayerLoot = true): void {
    ...
    this.syncResourceNodeFromFieldObject(fieldObject);
    if (fieldObject.resourceType) {
      markFactionResourceNodeDepleted(this.forceState, fieldObject.id, fieldObject.respawnDurationMs);
      this.forceStateDirty = true;
    }
  ...
  private spawnAsteroidLootDrops(fieldObject: SpaceFieldObject, x: number, y: number): void {
    ...
    if (fieldObject.resourceType === "starforged-alloy") {
      this.spawnSpaceItemPickup(x + Phaser.Math.Between(-24, 24), y + Phaser.Math.Between(-24, 24), createJunkLoot("starforged-alloy"));
      if (Math.random() <= 0.42) {
        this.spawnSpaceItemPickup(x + Phaser.Math.Between(-20, 20), y + Phaser.Math.Between(-20, 20), createJunkLoot("aetherium-ore"));
      }
      return;
    }
    if (fieldObject.resourceType === "aetherium-ore") {
      this.spawnSpaceItemPickup(x + Phaser.Math.Between(-20, 20), y + Phaser.Math.Between(-20, 20), createJunkLoot("aetherium-ore"));
      if (Math.random() <= 0.38) {
        this.spawnSpaceItemPickup(x + Phaser.Math.Between(-14, 14), y + Phaser.Math.Between(-14, 14), createJunkLoot("iron-ore"));
      }
      return;
    }
    if (fieldObject.resourceType === "scrap-ship-parts") {
      this.spawnSpaceItemPickup(x + Phaser.Math.Between(-18, 18), y + Phaser.Math.Between(-18, 18), createJunkLoot("scrap-ship-parts"));
      return;
    }
  ...
  private updateRadar(dt: number): void {
    ...
    if (this.radarRefreshTimerMs <= 0 || this.cachedRadarSources.length <= 0) {
      this.cachedRadarSources = this.collectRadarSources();
      this.radarRefreshTimerMs = RADAR_REFRESH_INTERVAL_MS;
    }
    this.radar.update(
  ...
  private isPrimeWorldAssistanceAvailable(planet: GalaxyPlanetRecord | null): boolean {
    return Boolean(
      planet?.homeworldRaceId
      && getRaceAllianceStatus(this.warState, planet.homeworldRaceId) !== "empire",
    );
  ...
  private buildPrimeWorldResourceRequest(planet: GalaxyPlanetRecord | null): PrimeWorldResourceRequest | null {
    if (!planet || !this.isPrimeWorldAssistanceAvailable(planet)) {
      return null;
    }

    const stockpile = this.forceState.systemStockpiles.find((candidate) => candidate.systemId === planet.systemId)?.resources ?? {};
    const donationOptions: Array<PrimeWorldResourceRequest> = [
      {
        planetId: planet.id,
    ...
    const thresholds: Record<FactionResourceType, number> = {
      "iron-ore": 60,
      "aetherium-ore": 16,
      "starforged-alloy": 4,
      "scrap-ship-parts": 26,
    ...
    return donationOptions.find((option) => (
      (stockpile[option.templateId] ?? 0) < thresholds[option.templateId]
    )) ?? donationOptions[donationOptions.length - 1] ?? null;
  }
  ...
  private getPrimeWorldMissionMenuState(interactionPlanet: GalaxyPlanetRecord | null = null): Partial<SpaceStationOverlayState> {
    ...
    if (this.primeWorldResourceRequest && interactionPlanet?.id === this.primeWorldResourceRequest.planetId) {
      const availableAmount = gameSession.getCargoResourceCount(this.primeWorldResourceRequest.templateId);
      return {
        missionActionLabel: "Donate\nResources",
        missionActionEnabled: availableAmount >= this.primeWorldResourceRequest.amount,
        missionActionSummary: `Prime foundries request ${this.primeWorldResourceRequest.amount} ${this.primeWorldResourceRequest.label}.\nCurrent cargo: ${availableAmount}\nReward: ${this.primeWorldResourceRequest.rewardCredits} credits + ${this.primeWorldResourceRequest.rewardXp} XP.`,
      };
    }
  ...
  private handleStationMissionAction(): void {
    ...
    if (this.primeWorldResourceRequest) {
      const request = this.primeWorldResourceRequest;
      const availableAmount = gameSession.getCargoResourceCount(request.templateId);
      if (availableAmount < request.amount) {
        this.stationOverlayStatusText = `Prime quartermasters still need ${request.amount} ${request.label}.`;
        const interaction = this.getNearestPrimeWorldInteraction();
        if (interaction) {
          this.stationOverlay?.update(this.buildPrimeWorldOverlayState(interaction.planet.name, interaction.planet));
        }
      ...
      const removed = gameSession.removeCargoResource(request.templateId, request.amount);
      if (removed < request.amount) {
        this.stationOverlayStatusText = "Cargo transfer failed. Verify your hold and try again.";
        const interaction = this.getNearestPrimeWorldInteraction();
        if (interaction) {
          this.stationOverlay?.update(this.buildPrimeWorldOverlayState(interaction.planet.name, interaction.planet));
        }
      ...
      depositFactionResourcesToSystem(this.forceState, request.systemId, { [request.templateId]: request.amount });
      gameSession.addCredits(request.rewardCredits);
      gameSession.addPlayerXp(request.rewardXp);
      this.forceStateDirty = true;
      this.primeWorldResourceRequest = this.buildPrimeWorldResourceRequest(
        this.galaxyPlanetsById.get(request.planetId) ?? null,
      );
      this.stationOverlayStatusText = `${request.label} delivered. Prime foundries rerouted the shipment to fleet production.`;
      const interaction = this.getNearestPrimeWorldInteraction();
      if (interaction) {
        this.stationOverlay?.update(this.buildPrimeWorldOverlayState(interaction.planet.name, interaction.planet));
      }
      this.pushStatusMessage(`${request.label} delivered to ${getGalaxySectorDisplayLabel(this.getCurrentGalaxySector(), this.warState)}.`, 6200);
      return;
    }
  ...
  private tryOpenNearestPrimeWorldComms(): boolean {
    ...
    this.releaseTouchControls();
    this.stationOverlayStationId = null;
    this.primeAssistanceMissionId = this.pickPrimeAssistanceMissionId(interaction.planet);
    this.primeWorldResourceRequest = this.buildPrimeWorldResourceRequest(interaction.planet);
    this.stationOverlayStatusText = this.isPrimeWorldAssistanceAvailable(interaction.planet)
      ? this.primeAssistanceMissionId
        ? "Prime World command has an assistance request ready."
        : this.primeWorldResourceRequest
          ? "Prime World command needs resource support for current fleet production."
          : "Prime World command has no urgent assistance request right now."
      : "Empire Prime World channels reject your assistance requests.";
  ...
  private pickPrimeAssistanceMissionId(interactionPlanet: GalaxyPlanetRecord | null = null): string | null {
    if (!this.isPrimeWorldAssistanceAvailable(interactionPlanet)) {
      return null;
    }
  ...
  getDebugSnapshot(): Record<string, unknown> {
    ...
    const currentZone = this.getCurrentGalaxyZone();
    const currentSystemStockpile = currentZone
      ? this.forceState.systemStockpiles.find((stockpile) => stockpile.systemId === currentZone.systemId) ?? null
      : null;
    const directReclaimTarget = this.getDirectReclaimLandingTarget();
    const nearbyAsteroidObjects = this.getNearbyFieldObjects(this.shipRoot.x, this.shipRoot.y, 2200)
      .filter((fieldObject) => fieldObject.kind === "asteroid");
    ...
    const nearbyResourceNodes = nearbyAsteroidObjects
      .map((fieldObject) => {
        const node = this.resourceNodesByFieldId.get(fieldObject.id) ?? null;
        return {
          id: fieldObject.id,
          resourceType: node?.resourceType ?? fieldObject.resourceType ?? "iron-ore",
          resourceVisualKind: node?.visualKind ?? fieldObject.resourceVisualKind,
          remainingYield: node?.remainingYield ?? fieldObject.resourceYield,
          totalYield: node?.totalYield ?? fieldObject.resourceYield,
          respawnRemainingMs: node?.depletedUntilSimTimeMs
            ? Math.max(0, Math.round(node.depletedUntilSimTimeMs - this.forceState.simulationTimeMs))
            : 0,
          miningSlots: node ? getFactionResourceNodeMiningSlotCount(node) : 1,
          x: Math.round(fieldObject.root.x),
          y: Math.round(fieldObject.root.y),
        };
      })
      .sort((left, right) => left.respawnRemainingMs - right.respawnRemainingMs)
      .slice(0, 8);
    ...
    const nearbyMiningExpeditions = this.getNearbyFactionShips(this.shipRoot.x, this.shipRoot.y, 2800)
      .filter((ship) => ship.shipRole === "miner-ship")
      .slice(0, 8)
      .map((ship) => {
        const forceShip = getFactionForceShipById(this.forceState, ship.id);
        const targetNode = ship.targetResourceNodeId
          ? getFactionResourceNodeById(this.forceState, ship.targetResourceNodeId)
          : null;
        const originSystem = ship.originSystemId ? this.galaxySystemsById.get(ship.originSystemId) ?? null : null;
        const escortCount = this.getNearbyFactionShips(ship.root.x, ship.root.y, 420)
          .filter((candidate) => candidate.id !== ship.id && candidate.factionId === ship.factionId && candidate.shipRole !== "miner-ship")
          .length;
        return {
          id: ship.id,
          factionId: ship.factionId,
          state: ship.miningState,
          cargo: { ...(forceShip?.cargo ?? ship.cargo) },
          cargoAmount: forceShip ? getFactionShipCargoAmount(forceShip) : Object.values(ship.cargo).reduce((sum, value) => sum + value, 0),
          originSystemId: ship.originSystemId,
          originSystemName: originSystem?.name ?? null,
          targetResourceNodeId: ship.targetResourceNodeId,
          targetResourceType: targetNode?.resourceType ?? null,
          targetRemainingYield: targetNode?.remainingYield ?? null,
          cargoCarrierShipId: ship.cargoCarrierShipId,
          escortCount,
          x: Math.round(ship.root.x),
          y: Math.round(ship.root.y),
    ...
    return {
      ...
      currentZone: currentZone
        ? {
            ...
            systemId: currentZone.systemId,
          }
        : null,
      currentSystemStockpile: currentSystemStockpile
        ? {
            systemId: currentSystemStockpile.systemId,
            zoneId: currentSystemStockpile.zoneId,
            raceId: currentSystemStockpile.raceId,
            kind: currentSystemStockpile.kind,
            resources: { ...currentSystemStockpile.resources },
          }
        : null,
      ...
      nearbyAsteroidCount: nearbyAsteroidObjects.length,
      nearestFieldObjects,
      nearbyResourceNodes,
      nearestShips,
      nearbyMiningExpeditions,
      smugglerRoutes,
    };
  }
}
